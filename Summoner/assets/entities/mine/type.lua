...
return {
	...
	ai = function(entity)
		...
		elseif entity.state == MINE_STATE_PREPARING then
			...
			entity:set_position(entity.parent.entity:get_position())
			entity.rotation = entity.rotation - 0.64fx
		...
		if entity.state ~= MINE_STATE_SPAWNING then
			...
			if pplaf.fxmath.abs(px - ex) < MINE_PLAYER_COLLISION and pplaf.fxmath.abs(py - ey) < MINE_PLAYER_COLLISION then
			...
			for id, owo in ipairs(OWO) do
				...
				if pplaf.fxmath.abs(ox - ex) < MINE_OWO_COLLISION and pplaf.fxmath.abs(oy - ey) < MINE_OWO_COLLISION and owo.state == 2 then
					owo.destroy()
					entity.damaged = 6
