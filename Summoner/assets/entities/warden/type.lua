local WARDEN_STATE_SPAWNING = 1
local WARDEN_STATE_ALIVE = 2
...
return {
	...
	collision = 90fx,
	spawn_time = 120,
	max_acceleration = 0.256fx,
	friction = 0.3600fx,
	
	weapons = {'overlord_laser', 'warden_mortar'},
	...
	constructor = function(entity, args)
		...
		entity.angle = 0fx
		entity.tick = 0
		entity.state = WARDEN_STATE_SPAWNING
		entity.damaged = 0
		entity.reward = false
	...
	destructor = function(entity)
		...
		if entity.reward then
			...
			ACHIEVEMENT_DESTROY_WARDEN = true
		end
	end,
	...
	ai = function(entity)
		...
		if entity.state == WARDEN_STATE_SPAWNING then
			
			entity.tick = entity.tick + 1
			if entity.tick == entity.type.spawn_time then
				entity.state = WARDEN_STATE_ALIVE
			end
			
		elseif entity.state == WARDEN_STATE_ALIVE then
			...
			if entity.invulnerable then
				...
				if pplaf.fxmath.abs(px - ex) < OVERLORD_PLAYER_COLLISION and pplaf.fxmath.abs(py - ey) < OVERLORD_PLAYER_COLLISION then
					...
					if entity.hp <= 0 then
						entity.reward = true
						entity:destroy()
						ACHIEVEMENT_PROGRESS_DESTROY_WARDEN_BY_MELEE = true
						return nil
					end
				end
				for id, owo in ipairs(OWO) do
