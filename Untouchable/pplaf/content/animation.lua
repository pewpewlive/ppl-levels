...
local function go_to_next_action(animation, animation_type)
  animation[i_action] = animation[i_action] + 1
  local next_action = animation_type.actions[animation[i_action]]
  if next_action then
    animation[i_action_param] = table.copy(next_action)
  else
    animation[i_action_param] = {100, animation_type.frames_per_tick + animation_type.frame_offset}
  end
end

local function action_wait_base(animation, animation_type)
  local timer = animation[i_action_param][2]
  if timer <= 1 then
    go_to_next_action(animation, animation_type)
  else
    animation[i_action_param][2] = timer - 1
  end
end
...
pplaf.animation = {
  ...
  modify_entity = function(entity) -- adds animation table in entity
    local animation_type = pplaf.animation.types[entity[i_type].animation]
    entity[i_animation] = {}
    entity[i_animation][i_type] = animation_type
    entity[i_animation][i_frame] = 0
    entity[i_animation][i_action] = 1
    entity[i_animation][i_action_param] = table.copy(animation_type.actions[1])
    entity[i_animation][i_variation_index] = 0
    entity[i_animation][i_variation_offset] = 0
  end,
  
  maintain = function(entity) -- maintains entity's animation
    local animation = entity[i_animation]
    local animation_type = animation[i_type]
    local template = animation_type.template
    local current_action = animation_type.actions[animation[i_action]]
    local current_action_param = animation[i_action_param]
    ...
    elseif action_type == action_list.animate then
      if animation[i_frame] == current_action_param[2] then
        go_to_next_action(animation, animation_type)
      end
      modify_frame = 1
    ...
    elseif action_type == action_list.loop then
      if animation[i_frame] == current_action_param[3] then
        animation[i_frame] = current_action[2]
      end
      modify_frame = 1
    elseif action_type == action_list.loopW then
      if current_action_param[4] == 1 and animation[i_frame] == current_action_param[3] - 1 then
        go_to_next_action(animation, animation_type)
      end
      if animation[i_frame] == current_action_param[3] then
        animation[i_frame] = current_action[2]
    ...
    elseif action_type == action_list.loop_back then
      if animation[i_frame] == current_action_param[2] then
        animation[i_frame] = current_action[3]
      end
      modify_frame = 2
    elseif action_type == action_list.loop_backW then
      if current_action_param[4] == 1 and animation[i_frame] == current_action_param[2] + 1 then
        go_to_next_action(animation, animation_type)
      end
      if animation[i_frame] == current_action_param[2] then
        animation[i_frame] = current_action[3]
    ...
    elseif action_type == action_list.loop_back_forthW then
      ...
      if current_action_param.loop_increment_param == 1 then
        df = current_action_param[3] - animation[i_frame]
      elseif current_action_param.loop_increment_param == 2 then
        df = animation[i_frame] - current_action_param[2]
      end
    ...
    elseif action_type == action_list.set_frame then
      animation[i_frame] = current_action_param[2]
      go_to_next_action(animation, animation_type)
    end
    ...
    if     template == 0 then -- animation is stored in 1 file; file contains A variation stored in order; every variation contains B frames
      ...
      frame = animation[i_variation_offset] + animation[i_frame]
    elseif template == 1 then -- every animation file contains 1 variation with B frames; A files
      path = animation_type.path .. animation[i_variation_index] + 1 .. '.lua'
      frame = animation[i_frame]
    elseif template == 2 then -- every animation file contains 1 frame of A variations(you store additional frames after animated frames); B files
      path = animation_type.path .. animation[i_frame] + animation_type.frame_offset + 1 .. '.lua'
      frame = animation[i_variation_index] * (animation_type.frames_per_tick + animation_type.frame_offset) -- OwO
    elseif template == 3 then -- every animation file contains 1 frame of 1 variation; A * B files
      path = animation_type.path .. animation[i_variation_offset] + animation[i_frame] + animation_type.frame_offset + 1 .. '.lua'
    end
    ...
    ::al_mmf::
    if modify_frame == 1 then
      animation[i_frame] = animation[i_frame] + animation_type.frames_per_tick + animation_type.frame_offset
    elseif modify_frame == 2 then
      animation[i_frame] = animation[i_frame] - animation_type.frames_per_tick - animation_type.frame_offset
    end
    ::al_me::
