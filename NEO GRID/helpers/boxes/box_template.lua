local player = require("/dynamic/helpers/player_helpers.lua")
...
function box.new(x, y, outer_mesh_info, inner_mesh_info, collision_callback, expiration)
  ...
  pewpew.customizable_entity_configure_music_response(id,{scale_x_start = 1fx,scale_x_end = 15fx/10fx,scale_y_start = 1fx,scale_y_end = 15fx/10fx})
  player.add_arrow(id,0xffff00ff)
  ...
  pewpew.customizable_entity_configure_music_response(id,{scale_x_start = 1fx,scale_x_end = 13fx/10fx,scale_y_start = 1fx,scale_y_end = 13fx/10fx})
  ...
  local function update_callback()
    ...
    if not scale_down then
        z = z + 1fx/20fx
    else  z = z - 1fx/20fx end
    pewpew.customizable_entity_set_mesh_angle(inner_mesh_id, inner_mesh_angle, 1fx, 0.2047fx, z)
