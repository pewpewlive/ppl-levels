...
function shield_box.new(x, y, resurrection_weapon_config)
  local b = box.new(x, y, {"/dynamic/helpers/boxes/box_graphics.lua", 0}, {"/dynamic/helpers/boxes/inner_box_graphics.lua", 0}, function(player_id, ship_id)
    ...
    local rn = fmath.random_int(1, 3)
    if rn == 1 then
      local new_message = floating_message.new(x, y, "Shield++", 1.5fx, 0xffff00ff, 3)
    elseif rn == 2 then
      local new_message = floating_message.new(x, y, "SHEEEESH", 1.5fx, 0xffff00ff, 3)
    elseif rn == 3 then
      local new_message = floating_message.new(x, y, "More life yall", 1.5fx, 0xffff00ff, 3)
    end
  end, 400)
end
