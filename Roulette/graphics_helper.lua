...
function helper.blue_theme_wall(mesh, center, r, g, b, a)
  local xchange = 450/2
  local ychange = 300/2
  ...
  for i = 0, 100, 50 do
    local a = a - i*2
    local color = r * 256 + g
    local color = color * 256 + b
    local color = color * 256 + a
    local a = {x - xchange + 150, y - ychange, z+i}
    local b = {x - xchange, y - ychange, z+i}
    local c2 = {x - xchange, y + ychange, z+i}
    local c = {x - xchange + 150, y + ychange, z+i}
    local d = {x + xchange - 150, y + ychange, z+i}
    local e = {x + xchange, y + ychange, z+i}
    local f = {x + xchange, y - ychange, z+i}
    local g = {x + xchange - 150, y - ychange, z+i}
    helper.add_line_to_mesh(mesh, {a, b, c2, c}, {color, color, color, color})
    helper.add_line_to_mesh(mesh,{d, e, f, g}, {color, color, color, color})
  end
end

function helper.add_star2(mesh, length, height, section_count, frequency, color_list)
  local turn = -1
  for i = 1, section_count-1 do
    ...
    for i = 1, frequency-1 do
      ...
      table.insert(vertices, {x + radius, y, z})
      table.insert(vertices, {x - radius, y, z})
      table.insert(colors, color)
      table.insert(colors, color)
      helper.add_line_to_mesh(mesh, vertices, colors)
...
function helper.add_squares(mesh, length, height, section_count, frequency, color_list)
  local turn = -1
  for i = 1, section_count-1 do
    local x = length/section_count*i
    local x2 = length/section_count*(i + 1)
    ...
    for i = 1, frequency-1 do
      ...
      table.insert(vertices, {x - radius, y + radius, z})
      table.insert(vertices, {x + radius, y + radius, z})
      table.insert(vertices, {x + radius, y - radius, z})
      table.insert(vertices, {x - radius, y - radius, z})
      table.insert(colors, color)
...
function helper.add_filled_rectangle2(mesh, center, height, length, color)
  local x = center[1]
  local y = center[2]
  local z = center[3]

  for i = -height, height do
    helper.add_line_to_mesh(mesh,{{x + length, y + i, z},{x - length, y + i, z}}, {color,color})
  end
