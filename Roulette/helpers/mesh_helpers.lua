...
function helper.add_square(mesh,center,length,height,color)
  local x = center[1]
  local y = center[2]
  local z = center [3]
  
  local a = {x - length/2,y - height/2,z}
  local b = {x + length/2,y - height/2,z}
  local c = {x + length/2,y + height/2,z}
  local d = {x - length/2,y + height/2,z}
...
function helper.add_horizontal_regular_rectangular_polygon_to_mesh(mesh, center, a, b, number_of_points, color, angle_offset)
  local compute = function(i)
    local angle = (i - 1) * 2 * math.pi / number_of_points
    local sin_angle, cos_angle = math.sincos(angle + angle_offset)
    return {center[1] + cos_angle * a, center[2] + sin_angle * b, center[3]}, color
  end
  helper.add_computed_segments_to_mesh(mesh, number_of_points, compute, true)
end
