...
local function modify_entity(entity, x, y, ...) -- entity modifications during it's init
  ...
  function entity:destroyA(...) -- destroyA maintains entity removal and calls destructor if possible
    ...
    pplaf.entity.get_group(entity.type.group)[entity.id] = nil
  end
end
...
pplaf.entity = {
  ...
  create = function(x, y, type, ...) -- create entity in position, with type and pass any parameters to constructor(if it exists)
    ...
    pplaf.entity.get_group(entity_type.group)[entity.id] = entity -- store entity
    return entity
  end,
  
  main = function()
    
    ENTITY_COUNTER = #pewpew.get_all_entities()
    pewpew.configure_player_hud(0, {top_left_line = string.format('#40ff40ffENTITY_COUNTER: %i', ENTITY_COUNTER)})
