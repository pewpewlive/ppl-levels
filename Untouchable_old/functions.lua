...
function check_collision_with_group(entity1, group_name)
  ...
  for _, entity2 in pairs(pplaf.entity.get_group(group_name)) do
    if entity2:get_is_alive() and not entity2:get_is_started_to_be_destroyed() then
      ...
      local dx = x2 - x1
      local dy = y2 - y1
      if __DEF_FMATH_SQRT(dx * dx + dy * dy) < entity1.type.radius + entity2.type.radius then
        return entity2
      end
    end
  end
...
function check_collision_with_player(entity)
  ...
  local dx = PLAYER_X - x
  local dy = PLAYER_Y - y
  if __DEF_FMATH_SQRT(dx * dx + dy * dy) < PLAYER.type.radius + entity.type.radius then
    return true
  end
end
...
function check_collision_with_player_lasers(entity)
  ...
  for _, laser in pairs(pplaf.entity.get_group'player_laser') do
    if laser.laser_state > -1 and laser:get_is_alive() then
      ...
      local dx = lx - ex
      local dy = ly - ey
      local angle = __DEF_FMATH_ATAN2(dy, dx) - laser.angle
      if __DEF_FMATH_ABS(angle) % TAU_FX >= PI_FX / 2fx and __DEF_FMATH_ABS(__DEF_FMATH_SQRT(dx * dx + dy * dy) * __DEF_FMATH_SINCOS(angle)) < entity.type.radius then
        return true
      end
    end
  end
